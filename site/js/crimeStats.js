...
export async function updateCrimeStats() {
    ...
    try {
        const crimeData = await fetchPoliceCrimeData(latitude, longitude, crimeType, date); // Await fetchPoliceCrimeData
        if (!crimeData || crimeData.length === 0) {
            crimeStats.textContent = 'No crime data found for the selected criteria.';
            return;
        }
